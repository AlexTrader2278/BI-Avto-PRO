...
ЗАДАЧА: Найди РЕАЛЬНУЮ марку и модель этого конкретного автомобиля.

Ищи в интернете: vin-info.com, vincheck.info, autocheck.com, autoteka.ru, vinautomobile.com, объявления Auto.ru/Drom.ru с этим VIN.

ВАЖНО:
- НЕ угадывай по структуре VDS-кода. Многие производители (особенно GM Korea KL1, Hyundai, Kia, Renault Russia) используют разные модели с похожими VDS-кодами.
- Если ты НЕ нашёл этот VIN в открытых базах И не уверен на 100% в модели — верни пустую строку в "model".
- Лучше пустое поле, чем неверная модель. Пользователь сможет ввести модель сам.

Верни ТОЛЬКО валидный JSON без пояснений:
{"make": "марка или пустая строка", "model": "конкретная модель или пустая строка"}`;
